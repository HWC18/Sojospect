...
def checkCookie(username, password):
    ...
    if cookie['secure'] != True or cookie['httpOnly'] != True:
        ...
        result = "'secure' value on cookie is set to True or 'httpOnly' value is set to True"
        print(result)

if seleniumBot.analyseConfig('scan_for_cookie_attribute_checking') != 1:
    print('Feature is not enabled')
    seleniumBot.driver.quit()
    quit()
else:
